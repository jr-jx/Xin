...
import { useShikiStore } from '../../stores/shiki'
...
const isCollapsed = ref(collapsible.value)
...
async function copy() {
	...
	try {
		...
		if (codeblock.value.textContent) {
			text = codeblock.value.textContent
		}
		else if (codeblock.value.textContent) {
			text = codeblock.value.textContent
		}
