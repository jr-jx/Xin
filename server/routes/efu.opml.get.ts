...
export default defineEventHandler(async (_e) => {
	...
	const opml = {
		$version: '2.0',
		head: {
			title: `${blogConfig.site.title}的友链订阅`,
			dateCreated: new Date(blogConfig.site.timeEstablished).toISOString(),
			dateModified: runtimeConfig.public.buildTime,
			ownerName: blogConfig.profile.name,
			ownerEmail: blogConfig.profile.email,
			ownerId: blogConfig.profile.homepage,
